...
export default class CartContents extends NavigationMixin(LightningElement) {
    ...
    handleMessage(message) {
        this.isSynced = message.isSynced;
        if(message.isCartAvailable) {
            if(message.cartId && message.effectiveAccountId && message.refreshCart) {
                if(!message.isCartActive) {
                    this.disableCartAction = true;
                }
                this.cartId = message.cartId;
                this.effectiveAccountId = message.effectiveAccountId;
                this.updateCartItems();
            } 
        } else {
            this.cartItems = [];
    ...
    handleCartUpdate(cartExists) {
        ...
        const payload = {
            isSynced: false,
            isCartAvailable: cartExists,
            isCartActive: true,
            cartId: this.cartId,
            effectiveAccountId: this.effectiveAccountId,
            sourceSystem: "Cart Content LWC"
        };
        publish(this.context, cartMsgChannel, payload);
    ...
    handleCartItemDelete(evt) {
        ...
        deleteCartItem({
            ...
            .then(() => {
                // this.template.querySelector('c-b2b-show-toast-message').showToast(B2B_This_Product_cannot_be_deleted,'error');
