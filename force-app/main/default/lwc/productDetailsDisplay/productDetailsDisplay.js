...
import createCase from '@salesforce/apex/B2BProductController.createCase';
import userId from '@salesforce/user/Id';
import getProductId from '@salesforce/apex/B2BProductController.getProductId';
...
{
    // Variables
    @api inventorydetails; 

    @track usrId = userId;

    @track desc = 'notify user when the product available';
    ...
    connectedCallback() {
        ...
        console.log('Product is in Stock ? : '  + this.inStock);
...
// Notify case creation when product is not available

// Refresh
// refreshInventory()
// {
//     getProductId({sku:this.sku})
//     .then((res)=>
//     {
//         console.log('Inside refresh');
//         getInventroyData({productId : res})
//         .then((result)=>{
...
handleNonAvl()
{
    createCase({descr:this.desc, userId:this.usrId})
    .then(result => {
        this.message = result;
        this.error = undefined;
        if(this.message !== undefined) {
            this.desc = '';
            this.template.querySelector('c-show-toast-l-w-r').showToast('You will be notified','success');
        }
    ...
    .catch(error => {
        ...
        this.error = error;
        this.template.querySelector('c-show-toast-l-w-r').showToast('Oops! Something went Wrong','error');
