...
export default class CartSummary extends LightningElement {
    ...
    @api
    recordId;
    @api webStoreId = '0ZE8G0000004J2eWAE';
    // cartadapter to get cat Id
    // @wire( CartSummaryAdapter, { cartStateOrId: 'current', webstoreId: '$webStoreId' } )
    // getCartSummary( { error, result } ) {
    //     if ( result ) {
    //         console.log( 'getCartSummary : ' + JSON.stringify( result ) );
    //         this.recordId = result.cartId;
    //     }
    //     else if ( error ) {
    ...
    async cart({ data, error }) {
        if (data) {
            console.log( 'getCartContent : ' + data.cartId);
